...
import { sendPostNotifications } from '@/data/Friendship';
...
const UserPhotos: React.FC<UserPhotosProps> = ({ user}) => {
    ...
    const handleModalSubmit = async (location: string, review: string, rating: number) => {
        ...
        // saves post and returns post id generated by the database 
        const postId = await savePost(user.id, userImage, location, review, rating);
        
        //make sure postId exists 
        if (postId != null) {
            await sendPostNotifications(user.id, postId);
        } else {
            console.error("Failed to generate postId.");
        }
